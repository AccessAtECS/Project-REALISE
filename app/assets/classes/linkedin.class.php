...
class linkedin {
  ...
  function getProfile($resource = "~") {
    ...
    $auth_header = preg_replace("/Authorization\: OAuth\,/", "Authorization: OAuth ", $auth_header); // commented out?
    // # Make sure there is a space between OAuth attribute
    $auth_header = preg_replace('/\"\,/', '", ', $auth_header); //commented out?
    if ($this->debug) {
      echo $auth_header;
    }
  ...
  function httpRequest($url, $auth_header, $method, $body = NULL) {
    ...
    if(count($auth_header) > 0) curl_setopt($curl, CURLOPT_HTTPHEADER, array($auth_header)); // Set the headers.
